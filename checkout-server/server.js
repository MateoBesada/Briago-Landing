...
app.post('/create_preference', async (req, res) => {
  try {
    // --- CORRECCIÓN AQUÍ: Aseguramos recibir 'additional_info' ---
    ...
    // --- CORRECCIÓN AQUÍ: Guardamos 'additional_info' ---
    ...
    const preference = {
      ...
      payer: {
        // --- CORRECCIÓN AQUÍ: Usamos 'name' y 'surname' ---
  ...
  try {
    if (req.body.type === 'payment') {
      ...
      if (payment.body.status === 'approved') {
        ...
        if (orderData) {
          const { items, payer, additional_info } = orderData; // Obtenemos todos los datos
          const totalAmount = payment.body.transaction_amount.toLocaleString('es-AR');

          // --- CAMBIO AQUÍ: Se añade 'modoVenta' al HTML ---
          const itemsHtml = items.map(item => `
            <tr style="border-bottom: 1px solid #eaeaea;">
              <td style="padding: 10px 5px; vertical-align: top;">
                ${item.nombre}
                <span style="font-size: 12px; color: #6b7280; display: block; text-transform: capitalize;">
                  (Modo: ${item.modoVenta || 'unidad'})
                </span>
              </td>
              <td style="padding: 10px 5px; text-align: center; vertical-align: top;">${item.cantidad}</td>
              <td style="padding: 10px 5px; text-align: right; font-weight: 600; vertical-align: top;">$${Number(item.precio).toLocaleString('es-AR')}</td>
            </tr>
          `).join('');
          ...
          // --- CAMBIO AQUÍ: Se añade 'modoVenta' al HTML del cliente ---
          const customerItemsHtml = items.map(item => `
            <tr style="border-bottom: 1px solid #eee;">
              <td style="padding: 10px 5px;">
                ${item.nombre}
                <span style="font-size: 12px; color: #6b7280; display: block; text-transform: capitalize;">
                  (Modo: ${item.modoVenta || 'unidad'}) x${item.cantidad}
                </span>
              </td>
              <td style="padding: 10px 5px; text-align: right; font-weight: 600;">$${(Number(item.precio) * item.quantity).toLocaleString('es-AR')}</td>
            </tr>
          `).join('');
